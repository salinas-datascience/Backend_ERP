    ...
    try:
        ...
        paginas_requeridas = [
            ...
            },
            {
                ...
                "ruta": "/analytics-ia",
                "titulo": "Analytics IA",
                "descripcion": "Análisis predictivo y optimización de stock con inteligencia artificial",
                "icono": "Brain",
                "orden": 7,
                "activa": True,
                "solo_admin": False
        ...
        # Actualizar títulos y orden de páginas existentes
        paginas_actualizadas = 0
        actualizaciones_paginas = {
            "repuestos": {"orden": 1},
            "proveedores": {"orden": 2},  
            "ordenes_compra": {"orden": 3},
            "maquinas": {"orden": 4},
            "modelos_maquinas": {"orden": 5},
            "plan_mantenimiento": {"orden": 6},
            "ordenes_trabajo": {"orden": 7},
            "mis_ordenes_trabajo": {
                "orden": 8,
                "titulo": "Mis OTs",
                "descripcion": "Órdenes de trabajo asignadas al usuario"
            },
            "analytics_ia": {"orden": 9},
            "dashboard_metricas": {"orden": 10},
            "historial": {
                "orden": 11,
                "titulo": "Historial de Consumo",
                "descripcion": "Ver historial de consumo y movimientos de repuestos"
            }
        }
        
        for pagina_existente in paginas_existentes:
            if pagina_existente.nombre in actualizaciones_paginas:
                updates = actualizaciones_paginas[pagina_existente.nombre]
                cambios = []
                
                if "orden" in updates:
                    pagina_existente.orden = updates["orden"]
                    cambios.append(f"orden: {updates['orden']}")
                    
                if "titulo" in updates:
                    titulo_anterior = pagina_existente.titulo
                    pagina_existente.titulo = updates["titulo"]
                    cambios.append(f"título: '{titulo_anterior}' -> '{updates['titulo']}'")
                    
                if "descripcion" in updates:
                    pagina_existente.descripcion = updates["descripcion"]
                    cambios.append("descripción actualizada")
                
                if cambios:
                    paginas_actualizadas += 1
                    print(f"✓ Página '{pagina_existente.titulo}' actualizada: {', '.join(cambios)}")
        ...
        if paginas_agregadas > 0 or paginas_actualizadas > 0:
            db.commit()
            if paginas_agregadas > 0:
                print(f"✓ {paginas_agregadas} páginas agregadas exitosamente")
            if paginas_actualizadas > 0:
                print(f"✓ {paginas_actualizadas} páginas actualizadas exitosamente")
        else:
            print("✓ Todas las páginas están actualizadas")
...
def create_system_pages(engine):
    ...
    try:
        ...
        # Definir páginas del sistema (ordenadas lógicamente)
        paginas_sistema = [
            # === INVENTARIO Y STOCK ===
            {
                "nombre": "repuestos",
